/**
 * anthropic.sdk.integration.test.ts
 *
 * Anthropic Vercel AI SDK end-to-end verification of the horizon-9 dual-patch.
 *
 ...
 * @ai-sdk/anthropic WITHOUT a per-file fetch: option being passed to the SDK
 ...
 *   The createAnthropic() options object MUST NOT contain a `fetch:` key of
 ...
 * default `npm test` never hits api.anthropic.com.
 *
 * Env-driven configuration (defaults match the rubric's documented values):
 *   - ANTHROPIC_API_KEY  required to opt in (above).
 *   - ANTHROPIC_BASE_URL optional; overrides the SDK's baseURL (useful for
 *                       proxy/staging endpoints). Default
 *                       `https://api.anthropic.com/v1/messages` for the URL
 *                       fragment assertion (the SDK appends `/messages` to
 *                       the provider baseURL).
 *   - ANTHROPIC_MODEL    optional; overrides the model id. Default
...
// stays correct when ANTHROPIC_BASE_URL points at a proxy or staging host.
...
function anthropicSdkSuite(ud: typeof import('undici')): void {
  ...
  test('one real @ai-sdk/anthropic generateText call is captured by the dual-patch (no per-file fetch shim)', async () => {
    ...
    // reference creation, env reads beyond ANTHROPIC_API_KEY).
    ...
    // NO-SHIM INVARIANT: createAnthropic receives ONLY apiKey (+ optional
    // baseURL from ANTHROPIC_BASE_URL). NO `fetch:` key of any kind. The
    ...
    const anthropicProvider = createAnthropic(providerOptions);
    ...
    try {
      ...
      expect(entry.model).toContain('claude');
...
function anthropicSdkSuitePlaceholder(): void {
...
if (hasKey) {
  if (undiciInstalled && undici !== undefined) {
    describe('anthropic SDK (dual-patch end-to-end verification)', () => {
      anthropicSdkSuite(undici);
    });
  } else {
    describe.skip(
      'anthropic SDK (skip: undici peer not installed — install undici@^6 to exercise the dual-patch)',
      anthropicSdkSuitePlaceholder,
    );
  }
} else {
  // No credential, no network egress: default `npm test` skips cleanly.
  describe.skip(
    'anthropic SDK (skip: ANTHROPIC_API_KEY not set)',
    anthropicSdkSuitePlaceholder,
  );
}
