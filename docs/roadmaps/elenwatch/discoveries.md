...
- 2026-08-31 | horizon 2 | ESM peer-require | src/peer-require.ts synthesizes the ESM `require` via createRequire anchored at process.cwd() (import.meta.url unusable without breaking the CJS emit); resolves optional peers from the app root — README Limitations should note fetch capture needs undici resolvable from the app's node_modules
